...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
  ...
  <xsl:template match="div[@id='uas-wrapper']">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="head">
    <xsl:copy>
      ...
      <meta name="service" content="Web hosting"/>
      <meta name="editorial" content="webcms"/>
      ...
      <meta name="DC.Creator.Address" content="webcms@admin.ox.ac.uk"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="a[@href]">
	...
	<xsl:choose>
	  <xsl:when test="starts-with(@href,'/bsp')">
	    <xsl:value-of select="replace(tei:mangleName(@href),'/bsp','')"/>
	  </xsl:when>
	  ...
	  <xsl:when  test="matches(@href,'(gif|jpeg|jpg|png)$')">
		...
		<xsl:message>curl -L -s -o "<xsl:value-of
		select="$target"/>" "<xsl:value-of select="concat('http://www1.admin.ox.ac.uk/',@href)"/>"</xsl:message>
	      </xsl:when>
	      <xsl:otherwise>
		<xsl:value-of select="replace($target,'images/','/sites/ithelp/files/images/')"/>
  ...
  <xsl:template match="img">
	...
	<xsl:message>curl -L -s -o <xsl:value-of select="($target,concat('http://www1.admin.ox.ac.uk/',@src))"/></xsl:message>
      </xsl:when>
      <xsl:when test="starts-with(@src,'../images')">
	<xsl:message>curl -L -s -o <xsl:value-of select="($target,concat('http://www1.admin.ox.ac.uk/',replace(@src,'../images','')))"/></xsl:message>
      </xsl:when>
      <xsl:otherwise>
